/**
 * Self-update for ReleaseMgr, working WITH the shared cross-plugin update hub in dwc-plugin-runtime.
 * On load it checks this fork's GitHub Releases and announces a newer build into the hub (a host like
 * Flexible Layouts shows it in the unified popup); otherwise it falls back to a one-off notification
 * and the About tab shows an in-context banner with one-click apply.
 */
import { ref } from "vue";
...
const OWNER = "jaysuk";
const REPO = "ReleaseMgr";
const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000;

const LS_ENABLED = "releaseMgr.updateCheck.enabled";
const LS_LAST = "releaseMgr.updateCheck.lastCheck";
const LS_DISMISSED = "releaseMgr.updateCheck.dismissed";
...
const t = (key: string, named?: Record<string, unknown>) =>
	i18n.global.t(`plugins.releaseMgr.updates.${key}`, named ?? {});
...
function syncHub(): void {
	...
	if (s?.updateAvailable && dismissedVersion.value !== s.latestVersion) {
		announceUpdate(PLUGIN_MANIFEST_ID, i18n.global.t("plugins.releaseMgr.title"), s);
	} else {
...
export async function applyUpdateNow(): Promise<void> {
	...
	try {
		...
		console.warn("[ReleaseMgr] update failed:", e);
